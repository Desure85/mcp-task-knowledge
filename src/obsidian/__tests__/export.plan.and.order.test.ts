...
let planExportProjectToVault: any
let exportProjectToVault: any
let createDoc: any
let updateDoc: any
let createTask: any
let updateTask: any
...
async function readAllFilesRec(root: string): Promise<Record<string, string>> {
...
beforeAll(async () => {
  ...
  await updateTask(PROJECT, t1.id, { status: 'pending' as any })
  await updateTask(PROJECT, t2.id, { status: 'in_progress' as any, priority: 'high' as any })
  await updateTask(PROJECT, t3.id, { status: 'completed' as any })
})
...
describe('obsidian export — plan (merge/replace, filters, keepOrphans)', () => {
  ...
  it('replace plan marks target dirs for deletion', async () => {
    ...
    expect(plan.willDeleteDirs.some((p: string) => p.endsWith(path.join(PROJECT, 'Knowledge')))).toBe(true)
    expect(plan.willDeleteDirs.some((p: string) => p.endsWith(path.join(PROJECT, 'Tasks')))).toBe(true)
  })
})
