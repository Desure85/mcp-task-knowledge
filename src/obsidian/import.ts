...
interface ImportOptions {
  ...
  // Import strategy:
  // - 'merge': keep existing content; update existing items matched by title and create missing ones.
  // - 'replace': delete existing items in the selected categories (knowledge/tasks) for the project, then import fresh.
  strategy?: 'merge' | 'replace';
  // Merge conflict handling strategy (applies when strategy === 'merge').
  // - 'overwrite' (default): update existing by title
  // - 'append': always create new items; ignore title collisions
  // - 'skip': do not create/update on collision
  // - 'fail': abort import on first collision
  mergeStrategy?: 'overwrite' | 'append' | 'skip' | 'fail';
...
export interface ImportPlan {
  deletes: { knowledge: number; tasks: number };
...
async function readMarkdown(filePath: string): Promise<{ frontmatter: any; content: string } | null> {
  try {
...
function buildPathSets(projectRoot: string, includePaths?: string[], excludePaths?: string[]): { includeSet?: Set<string>; excludeSet?: Set<string> } {
  const includeSet = includePaths && includePaths.length > 0
    ? new Set(
        fg.sync(includePaths, { cwd: projectRoot, dot: true, onlyFiles: true })
          .map((p) => toPosix(p))
      )
    : undefined;
  const excludeSet = excludePaths && excludePaths.length > 0
    ? new Set(
        fg.sync(excludePaths, { cwd: projectRoot, dot: true, onlyFiles: true })
          .map((p) => toPosix(p))
      )
    : undefined;
...
function passesPathFilters(relPath: string, includeSet?: Set<string>, excludeSet?: Set<string>): boolean {
  const rp = toPosix(relPath);
  if (excludeSet && excludeSet.has(rp)) return false;
  if (includeSet) return includeSet.has(rp);
  return true;
}

function arrayIntersectAny(a?: string[], b?: string[]): boolean {
  if (!a || !b || a.length === 0 || b.length === 0) return false;
  const bs = new Set(b.map((x) => x.toLowerCase()));
  for (const x of a) {
    if (bs.has(String(x).toLowerCase())) return true;
  }
  return false;
}

function normalizeStr(x?: string): string | undefined {
  return typeof x === 'string' ? x.toLowerCase() : undefined;
}

function matchKnowledgeFilters(front: any, typeFromPath: string | undefined, opts?: ImportOptions): boolean {
  const tags: string[] | undefined = Array.isArray(front?.tags) ? front.tags : undefined;
  const typeVal: string | undefined = normalizeStr(front?.type) || normalizeStr(typeFromPath);
  // excludeTags has precedence
  if (opts?.excludeTags && arrayIntersectAny(tags?.map((t) => t.toLowerCase()), opts.excludeTags.map((t) => t.toLowerCase()))) return false;
  if (opts?.includeTags && !arrayIntersectAny(tags?.map((t) => t.toLowerCase()), opts.includeTags.map((t) => t.toLowerCase()))) return false;
  if (opts?.includeTypes && opts.includeTypes.length > 0) {
    ...
    if (!allow.has(typeVal)) return false;
  }
  return true;
}

function matchTaskFilters(front: any, opts?: ImportOptions): boolean {
  const tags: string[] | undefined = Array.isArray(front?.tags) ? front.tags : undefined;
  const status: string | undefined = normalizeStr(front?.status);
  const priority: string | undefined = normalizeStr(front?.priority);
  if (opts?.excludeTags && arrayIntersectAny(tags?.map((t) => t.toLowerCase()), opts.excludeTags.map((t) => t.toLowerCase()))) return false;
  if (opts?.includeTags && !arrayIntersectAny(tags?.map((t) => t.toLowerCase()), opts.includeTags.map((t) => t.toLowerCase()))) return false;
  if (opts?.includeStatus && opts.includeStatus.length > 0) {
    const allow = new Set(opts.includeStatus.map((s) => s.toLowerCase()));
    if (!status || !allow.has(status)) return false;
  }
  if (opts?.includePriority && opts.includePriority.length > 0) {
    const allow = new Set(opts.includePriority.map((p) => p.toLowerCase()));
    if (!priority || !allow.has(priority)) return false;
  }
  return true;
}

export async function planImportProjectFromVault(project?: string, opts?: ImportOptions): Promise<ImportPlan> {
  ...
  const mergeStrategy: 'overwrite' | 'append' | 'skip' | 'fail' = (() => {
    ...
    if (strategy === 'merge') return (opts?.overwriteByTitle !== false) ? 'overwrite' : 'skip';
    return 'overwrite';
  })();
  ...
  const existingDocsByTitle = new Map<string, string>();
  const existingTasksByTitle = new Map<string, string>();
  for (const m of existingDocs) existingDocsByTitle.set(m.title, m.id);
  for (const t of existingTasks) existingTasksByTitle.set(t.title, t.id);
  ...
  if (strategy === 'replace') {
    if (doKnowledge) plan.deletes.knowledge = existingDocs.length;
    if (doTasks) plan.deletes.tasks = existingTasks.length;
  ...
  if (doKnowledge) {
    ...
    if (await pathExists(knowledgeDir)) {
      async function walkKnowledge(dir: string, parentId: string | null) {
        ...
        if (await pathExists(indexPath) && passesPathFilters(relIndex, includeSet, excludeSet)) {
          ...
          if (md && matchKnowledgeFilters(md.frontmatter, path.basename(path.dirname(dir)), opts)) {
            const titleFromFM: string | undefined = md.frontmatter?.title;
            ...
            if (existingDocsByTitle.has(inferredTitle)) {
              ...
              if (plan.conflicts!.sampleTitles!.knowledge.length < 10) plan.conflicts!.sampleTitles!.knowledge.push(inferredTitle);
              ...
              plan.creates.knowledge++;
              if (!existingDocsByTitle.has(inferredTitle)) existingDocsByTitle.set(inferredTitle, 'planned');
            }
          }
        }
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            if (!passesPathFilters(rel, includeSet, excludeSet)) continue;
            const md = await readMarkdown(full);
            if (!md) continue;
            if (!matchKnowledgeFilters(md.frontmatter, path.basename(path.dirname(dir)), opts)) continue;
            const titleFromFM: string | undefined = md.frontmatter?.title;
  ...
  if (doTasks) {
    ...
    if (await pathExists(tasksDir)) {
      async function walkTasks(dir: string, parentId: string | null) {
        const indexPath = path.join(dir, 'INDEX.md');
        const relIndex = toPosix(path.relative(projectRoot, indexPath));
        if (await pathExists(indexPath) && passesPathFilters(relIndex, includeSet, excludeSet)) {
          const md = await readMarkdown(indexPath);
          if (md && matchTaskFilters(md.frontmatter, opts)) {
            ...
            if (existingTasksByTitle.has(inferredTitle)) {
              ...
              if (plan.conflicts!.sampleTitles!.tasks.length < 10) plan.conflicts!.sampleTitles!.tasks.push(inferredTitle);
              if (mergeStrategy === 'overwrite') {
                plan.updates.tasks++;
              } else if (mergeStrategy === 'append') {
                plan.creates.tasks++;
              ...
              plan.creates.tasks++;
              if (!existingTasksByTitle.has(inferredTitle)) existingTasksByTitle.set(inferredTitle, 'planned');
            }
          }
        }
        const entries = await fs.readdir(dir, { withFileTypes: true });
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            const md = await readMarkdown(full);
            if (!md) continue;
            if (!matchTaskFilters(md.frontmatter, opts)) continue;
            const titleFromFM: string | undefined = md.frontmatter?.title;
            const inferredTitle = titleFromFM || sanitizeTitle(path.basename(entry.name, '.md'));
            if (existingTasksByTitle.has(inferredTitle)) {
              plan.conflicts!.tasks++;
      ...
      for (const e of rootEntries) {
        if (e.isDirectory()) await walkTasks(path.join(tasksDir, e.name), null);
      }
    }
  }
...
export async function importProjectFromVault(project?: string, opts?: ImportOptions): Promise<ImportResult> {
  const cfg = loadConfig();
  const vaultRoot = cfg.obsidian.vaultRoot;
  const pfx = project || '';
  const projectRoot = path.join(vaultRoot, pfx ? pfx : '');
  ...
  if (strategy === 'replace') {
    if (doKnowledge) {
      ...
      for (const m of metas) await deleteDocPermanent(m.project, m.id);
    }
    if (doTasks) {
      const tasks = await listTasks({ project });
      for (const t of tasks) await deleteTaskPermanent(t.project, t.id);
    }
  }
  ...
  if (doKnowledge) {
    const metas = await listDocs({ project });
    for (const m of metas) existingDocsByTitle.set(m.title, m.id);
  }
  if (doTasks) {
    const tasks = await listTasks({ project });
    for (const t of tasks) existingTasksByTitle.set(t.title, t.id);
  }
  ...
  if (doKnowledge) {
    const knowledgeDir = path.join(projectRoot, 'Knowledge');
    if (await pathExists(knowledgeDir)) {
      async function importKnowledgeFolder(dir: string, parentId: string | null) {
        ...
        if (await pathExists(indexPath)) {
          ...
          if (passesPathFilters(relIndex, includeSet, excludeSet)) {
            ...
            if (md && matchKnowledgeFilters(md.frontmatter, path.basename(path.dirname(dir)), opts)) {
              const titleFromFM: string | undefined = md.frontmatter?.title;
              const inferredTitle = titleFromFM || sanitizeTitle(path.basename(dir));
              const typeVal: string | undefined = md.frontmatter?.type || path.basename(path.dirname(dir));
              const tags: string[] | undefined = Array.isArray(md.frontmatter?.tags) ? md.frontmatter.tags : undefined;
              const source: string | undefined = typeof md.frontmatter?.source === 'string' ? md.frontmatter.source : undefined;
              if (existingDocsByTitle.has(inferredTitle)) {
                if (mergeStrategy === 'overwrite') {
  ...
  if (doTasks) {
    const tasksDir = path.join(projectRoot, 'Tasks');
    if (await pathExists(tasksDir)) {
      async function importTaskFolder(dir: string, parentId: string | null) {
        let currentParentId = parentId;
        const indexPath = path.join(dir, 'INDEX.md');
        if (await pathExists(indexPath)) {
          const relIndex = toPosix(path.relative(projectRoot, indexPath));
          if (passesPathFilters(relIndex, includeSet, excludeSet)) {
            ...
            if (md && matchTaskFilters(md.frontmatter, opts)) {
              const titleFromFM: string | undefined = md.frontmatter?.title;
              const inferredTitle = titleFromFM || sanitizeTitle(path.basename(dir));
              const status: string | undefined = md.frontmatter?.status;
              const priority: string | undefined = md.frontmatter?.priority;
              const tags: string[] | undefined = Array.isArray(md.frontmatter?.tags) ? md.frontmatter.tags : undefined;
              const links: string[] | undefined = Array.isArray(md.frontmatter?.links) ? md.frontmatter.links : undefined;
      ...
      const rootEntries = await fs.readdir(tasksDir, { withFileTypes: true });
      for (const e of rootEntries) {
        if (e.isDirectory()) await importTaskFolder(path.join(tasksDir, e.name), null);
      }
    }
  }
