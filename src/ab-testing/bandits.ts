import { Aggregates } from './storage.js';

export interface BanditParams {
  epsilon?: number; // for epsilon-greedy
}

export function pickWithEpsilonGreedy(variants: string[], stats: Aggregates, params?: BanditParams): string {
  const eps = params?.epsilon ?? 0.1;
  if (variants.length === 0) throw new Error('No variants to pick from');
  // Explore
  if (Math.random() < eps) {
