...
import { createTask, listTasks, updateTask, closeTask, listTasksTree, archiveTask, trashTask, restoreTask, deleteTaskPermanent, getTask, getTaskSubtree, getDirectChildren, closeTaskWithCascade, MAX_TASK_DEPTH } from "./storage/tasks.js";
...
async function main() {
  ...
  server.registerResource(
    ...
    {
      title: "Task Actions",
      description: "Actions via path: task://action/<project>/<id>/{start|complete|close|...}. Query template also available: task://action{?project,id,action,status}",
      mimeType: "application/json"
    },
    taskResourceHandler
  );
  ...
  if (TOOL_RES_ENABLED) server.registerResource(
    ...
    {
      title: "Tool Schema",
      description: "Read-only metadata for tools (use tools.run to execute)",
      mimeType: "application/json",
    },
    async (uri) => {
      const href = uri.href;
  ...
  // tasks.create — create a single task
  server.registerTool(
    "tasks_create",
    {
      title: "Create Task",
      description:
        "Create a single task. Optionally set parentId to create a subtask. " +
        `Maximum nesting depth is ${MAX_TASK_DEPTH} levels.`,
      inputSchema: {
        project: z.string().default(DEFAULT_PROJECT),
        title: z.string().min(1),
        description: z.string().optional(),
  ...
  // tasks.update — update a single task
  server.registerTool(
    "tasks_update",
    {
      title: "Update Task",
      description:
        "Update a single task by id. Can change any field except id, project, createdAt. " +
        "Setting parentId moves the task in the hierarchy (cycle and depth protected). " +
        "Set parentId to null to detach from parent (make root).",
      inputSchema: {
        project: z.string().default(DEFAULT_PROJECT),
        id: z.string().min(1),
        title: z.string().optional(),
  ...
  // tasks.add_subtask — convenience tool to create a child task
  server.registerTool(
    "tasks_add_subtask",
    {
      title: "Add Subtask",
      description:
        "Create a subtask under a parent task. Shorthand for tasks_create with parentId. " +
        `Maximum nesting depth is ${MAX_TASK_DEPTH} levels.`,
      inputSchema: {
        project: z.string().default(DEFAULT_PROJECT),
  ...
  // tasks.get_subtree — get a specific task + all descendants as a tree
  server.registerTool(
    "tasks_get_subtree",
    {
      title: "Get Task Subtree",
      description:
        "Get a specific task and all its descendants as a hierarchical tree. " +
        "Useful for inspecting a branch of the task hierarchy.",
      inputSchema: {
        project: z.string().default(DEFAULT_PROJECT),
        id: z.string().min(1).describe("Root task ID of the subtree"),
        maxDepth: z.number().int().min(1).max(10).optional()
          .describe("Limit depth of the subtree (default: unlimited, max: 10)"),
      },
    },
    async ({ project, id, maxDepth }) => {
      ...
      // Apply maxDepth truncation if requested
  ...
  // tasks.get_children — get direct children of a task (one level only)
  server.registerTool(
    "tasks_get_children",
    {
  ...
  // tasks.close — close a single task, optionally cascade to descendants
  server.registerTool(
    "tasks_close",
    {
      title: "Close Task",
      description:
        ...
        "Optionally cascade the close to all descendants (subtasks, sub-subtasks, etc.).",
      inputSchema: {
        project: z.string().default(DEFAULT_PROJECT),
        id: z.string().min(1),
        cascade: z.boolean().default(false).optional()
          .describe("If true, also close all descendant tasks"),
      },
    },
  ...
  // ===== Search Resources and Templates =====
  const handleSearchTasksHref = async (href: string) => {
    ...
    if (paramsM) {
      ...
      let params: any = {};
      try { params = JSON.parse(Buffer.from(normalizeBase64(rawParams), 'base64').toString('utf8')); }
  ...
  try {
    server.registerResource(
      ...
      makeResourceTemplate('search://tasks/{project}/{params}'),
  ...
  try {
    server.registerResource(
      ...
      makeResourceTemplate('search://knowledge/{project}/{params}'),
