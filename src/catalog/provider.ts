...
function pickRemote(cfg: CatalogConfig): boolean {
  if (cfg.mode === "remote") return true;
  if (cfg.mode === "embedded") return false;
  // hybrid
...
async function remoteQuery(baseUrl: string, q: ServiceQuery, timeoutMs: number): Promise<ServicePage> {
  const u = new URL("/api/services", baseUrl);
  const p = u.searchParams;
  if (q.search) p.set("search", q.search);
  if (q.component) p.set("component", q.component);
  for (const o of ensureArray(q.owner) || []) p.append("owner", o);
  for (const t of ensureArray(q.tag) || []) p.append("tag", t);
...
async function remoteHealth(baseUrl: string, timeoutMs: number): Promise<boolean> {
...
export function createServiceCatalogProvider(cfg: CatalogConfig): ServiceCatalogProvider {
  const useRemotePreferred = pickRemote(cfg);

  // Simple embedded store backed by file or memory
  type EmbeddedState = {
    initialized: boolean;
    items: ServiceItem[];
    filePath?: string;
    lastLoadedAt?: number; // mtimeMs snapshot
  ...
  function reloadIfNeeded(): void {
    ...
    try {
      ...
      if (!embedded.initialized || embedded.lastLoadedAt !== m) {
        ...
        const items: ServiceItem[] = Array.isArray(data) ? data : Array.isArray(data?.items) ? data.items : [];
  ...
  return {
    ...
    async queryServices(q: ServiceQuery): Promise<ServicePage> {
      // Embedded implementation: file/memory backend
      const tryRemote = async () => {
        if (!cfg.remote.enabled || !cfg.remote.baseUrl) throw new Error("remote disabled");
        return remoteQuery(cfg.remote.baseUrl, q, cfg.remote.timeoutMs);
      };

      const tryEmbedded = async () => {
        if (!cfg.embedded.enabled) throw new Error("embedded disabled");
      ...
      if (cfg.mode === "remote") return tryRemote();
      if (cfg.mode === "embedded") return tryEmbedded();
    ...
    async health() {
      ...
      if (useRemotePreferred) {
        ...
        if (cfg.embedded.store === "file") {
          ...
          if (okEmb) return { ok: true, source: "embedded", detail };
        } else {
          return { ok: true, source: "embedded", detail };
        }
        // fallback to remote
    ...
    async upsertServices(items: ServiceItem[]) {
      ...
      for (const it of embedded.items) byId.set(it.id, it);
      for (const raw of items || []) {
        const it: ServiceItem = { ...raw } as any;
      ...
      if (cfg.embedded.store === 'file') {
        const p = embedded.filePath;
        if (!p) throw new Error('embedded.filePath is required for file store');
        try {
          ...
          throw new Error(`persist failed: ${String((e as any)?.message || e)}`);
        }
      }
