...
export async function planExportProjectToVault(project, opts) {
    ...
    if (doKnowledge) {
        ...
        const kById = new Map(metas.map(m => [m.id, m]));
        const kSelected = new Set();
        function kMatches(m) {
            const ua = m.updatedAt ? new Date(m.updatedAt) : undefined;
            if (updatedFrom && ua && ua < updatedFrom)
                return false;
            if (updatedTo && ua && ua > updatedTo)
                return false;
            const tags = m.tags;
            if (excludeTags && tags && tags.some(t => excludeTags.has(t)))
                return false;
            if (includeTags && (!tags || !tags.some(t => includeTags.has(t))))
                return false;
            const t = m.type;
            if (excludeTypes && t && excludeTypes.has(t))
                return false;
            if (includeTypes && (!t || !includeTypes.has(t)))
                return false;
            return true;
        }
    ...
    if (doTasks) {
        ...
        const tById = new Map();
        const tParent = new Map();
        for (const t of list) {
            tById.set(t.id, t);
            tParent.set(t.id, t.parentId ?? null);
        }
        function tMatches(n) {
            if (includeStatus && !includeStatus.has(n.status))
                return false;
            if (includePriority && !includePriority.has(n.priority))
                return false;
            const tags = n.tags || [];
            if (excludeTags && tags.some(t => excludeTags.has(t)))
                return false;
        ...
        const tSelected = new Set();
        for (const t of tById.values())
            if (tMatches(t))
                tSelected.add(t.id);
        const tClosure = new Set();
        if (keepOrphans) {
            for (const id of tById.keys())
                tClosure.add(id);
        }
        else {
            const visited = new Set();
...
export async function exportProjectToVault(project, opts) {
    const cfg = loadConfig();
    const vaultRoot = cfg.obsidian.vaultRoot;
    const pfx = project || '';
    const projectRoot = path.join(vaultRoot, pfx ? pfx : '');
    ...
    let kMap = new Map();
    let kChildren = new Map();
    let kSelected = new Set();
    let kClosure = new Set();
    if (doKnowledge) {
        const metas = await listDocs({ project, includeArchived });
        kMap = new Map();
        ...
        // Deterministic order
        for (const arr of kChildren.values())
    ...
    async function writeKnowledgeRec(id, ancestors) {
        if (!kClosure.has(id))
            return;
        ...
        const baseSegments = [knowledgeDir, typeFolder(doc.type)];
        for (const a of ancestors)
            baseSegments.push(sanitize(a.title));
        ...
        const fm = toFrontmatter({ id: doc.id, project: doc.project, title: doc.title, tags: doc.tags, source: doc.source, updatedAt: doc.updatedAt, parentId: doc.parentId, type: doc.type, structuralOnly: structuralOnly || undefined });
        const body = structuralOnly ? `${fm}\n` : `${fm}\n\n${doc.content || ''}`;
        await writeText(outPath, body);
        knowledgeCount++;
        for (const k of kids)
            await writeKnowledgeRec(k.id, [...ancestors, meta]);
    }
    ...
    async function writeTaskNode(node, ancestors) {
        if (!tClosure.has(node.id))
            return;
        const childrenAll = node.children || [];
        const children = childrenAll
            .filter((ch) => tClosure.has(ch.id))
