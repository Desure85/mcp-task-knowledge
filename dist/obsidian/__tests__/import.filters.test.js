...
async function buildVaultStructure(root) {
    const prjRoot = path.join(root, PROJECT);
    // Knowledge
    await writeFile(path.join(prjRoot, 'Knowledge', 'AreaA', 'INDEX.md'), md({ title: 'AreaA', tags: ['ml'], type: 'note' }, '# AreaA'));
    await writeFile(path.join(prjRoot, 'Knowledge', 'AreaA', 'NoteOne.md'), md({ title: 'NoteOne', tags: ['ml'], type: 'note' }, 'Content One'));
    await writeFile(path.join(prjRoot, 'Knowledge', 'AreaB', 'INDEX.md'), md({ title: 'AreaB', tags: ['private'], type: 'spec' }, '# AreaB'));
    await writeFile(path.join(prjRoot, 'Knowledge', 'AreaB', 'Skip.md'), md({ title: 'Skip', tags: ['private'], type: 'spec' }, 'Skip content'));
    // Tasks
    await writeFile(path.join(prjRoot, 'Tasks', 'Box1', 'INDEX.md'), md({ title: 'Box1', tags: ['work'], status: 'pending', priority: 'high' }, '# Box1'));
    await writeFile(path.join(prjRoot, 'Tasks', 'Box1', 'T1.md'), md({ title: 'T1', tags: ['work'], status: 'pending', priority: 'medium' }, 'Task 1'));
}
beforeAll(async () => {
