...
async function buildVault(root) {
    ...
    await writeFile(path.join(prjRoot, 'Tasks', 'Alpha', 'INDEX.md'), md({ title: 'Alpha', tags: ['team'], status: 'completed', priority: 'medium' }, '# Alpha'));
    await writeFile(path.join(prjRoot, 'Tasks', 'Alpha', 'T_done.md'), md({ title: 'T_done', tags: ['team'], status: 'completed', priority: 'low' }, 'Task done'));
    await writeFile(path.join(prjRoot, 'Tasks', 'Beta', 'INDEX.md'), md({ title: 'Beta', tags: ['team'], status: 'pending', priority: 'high' }, '# Beta'));
    await writeFile(path.join(prjRoot, 'Tasks', 'Beta', 'T_high.md'), md({ title: 'T_high', tags: ['team'], status: 'in_progress', priority: 'high' }, 'High prio task'));
}
beforeAll(async () => {
