...
async function buildVault(root) {
    ...
    await writeFile(path.join(prjRoot, 'Knowledge', 'AreaC', 'INDEX.md'), md({ title: 'AreaC', tags: ['ml', 'public'], type: 'note' }, '# AreaC'));
    await writeFile(path.join(prjRoot, 'Knowledge', 'AreaC', 'Note2.md'), md({ title: 'Note2', tags: ['ml', 'public'], type: 'note' }, 'Note2 body'));
    await writeFile(path.join(prjRoot, 'Knowledge', 'AreaD', 'INDEX.md'), md({ title: 'AreaD', tags: ['private'], type: 'spec' }, '# AreaD'));
    await writeFile(path.join(prjRoot, 'Knowledge', 'AreaD', 'Note3.md'), md({ title: 'Note3', tags: ['private'], type: 'spec' }, 'Note3 body'));
}
beforeAll(async () => {
