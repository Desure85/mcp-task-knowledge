...
export class EmbeddingsCache {
    ...
    textHash(text) { return EmbeddingsCache.hashText(text); }
    async get(id, expectedHash) {
        ...
        if (this.persist && this.dir) {
            try {
                ...
                if (meta.hash !== expectedHash || meta.dims !== this.dims)
                    return undefined;
    ...
    async set(id, hash, vec) {
        ...
        if (file) {
            try {
                ...
                const meta = { id, hash, dims: this.dims, bytes: vec.byteLength, file };
