...
export class NoopVectorAdapter {
    async search(_query, _items, opts) {
        return [];
    }
}
export class OnnxVectorAdapter {
    constructor(dim) {
        this.ready = false;
        this.maxLen = 256;
        this.dim = dim;
        ...
        try {
            ...
            try {
                ...
                if (fs.existsSync(metaPath)) {
                    ...
                    if (!Number.isNaN(maxp) && !cfg.embeddings.maxLen)
                        this.maxLen = Math.min(512, Math.max(16, maxp));
                }
            }
            catch { }
            ...
            if (useGpu && ort?.InferenceSession) {
                ...
                const probeCuda = async (modelPath) => {
                    try {
                        // Allow disabling the probe via env
                        if (process.env.ORT_SAFE_CUDA_PROBE === '0')
                            return true;
                        ...
                        if (res.signal) {
                            if (DEBUG)
                                console.warn('[vector][dbg] CUDA probe crashed with signal:', res.signal);
                            return false;
                        }
                        if (res.status !== 0) {
                            if (DEBUG)
                                console.warn('[vector][dbg] CUDA probe exited with code:', res.status, 'stderr:', res.stderr?.toString?.());
                            return false;
                        }
                        return true;
                    }
                        ...
                        if (DEBUG)
                            console.warn('[vector][dbg] CUDA probe exception:', e instanceof Error ? e.message : String(e));
                        return false;
                    }
                };
                const errs = [];
                for (const ep of epOrder) {
                    try {
                        if (DEBUG)
                            console.log(`[vector][dbg] trying EP=${ep} with modelSource=path`);
                        if (ep === 'cuda') {
                            // Be explicit about device selection for CUDA EP
                            const ok = await probeCuda(modelSource);
                            if (!ok)
                                throw new Error('CUDA probe failed');
                            createOpts.executionProviders = [{ name: 'cuda', deviceId: 0 }];
                            if (DEBUG)
                                console.log('[vector][dbg] CUDA EP options:', createOpts.executionProviders[0]);
                        }
                        else {
                        ...
                        if (DEBUG)
                            console.log(`[vector][dbg] EP=${ep} init OK`);
                        break;
                    }
                    catch (e) {
                        ...
                        errs.push(`[${ep}] ${msg}`);
                        if (DEBUG)
                            console.warn(`[vector][dbg] EP=${ep} failed:`, msg);
                    }
                }
                if (!this.session) {
            ...
            if (useGpu) {
                try {
                    const providers = this.session?.executionProvider ?? ort?.getAvailableExecutionProviders?.();
            ...
            const encodeBatch = async (texts) => {
                ...
                if (this.tokenizerKind === 'hf-tokenizers') {
                    ...
                    for (let b = 0; b < batch; b++) {
                        const ids = enc[b].ids.slice(0, seqLen);
                        const mask = Array(ids.length).fill(1);
                        while (ids.length < seqLen)
                            ids.push(0);
                        while (mask.length < seqLen)
                            mask.push(0);
                    ...
                    const toNumberArray = (arr) => {
                        ...
                        if (Array.isArray(arr))
                            return arr.map((v) => Number(v?.valueOf?.() ?? v));
                        // Object with 'data' field
                        if (arr?.data)
                            return toNumberArray(arr.data);
                        return [];
                    };
                    for (let b = 0; b < batch; b++) {
                        const raw = texts[b];
                        ...
                        const ids = toNumberArray(out.input_ids).slice(0, seqLen);
                        const mask = toNumberArray(out.attention_mask).slice(0, seqLen);
                        if (process?.env?.DEBUG_VECTOR) {
                            if (ids.some((v) => Number.isNaN(v)))
                                console.warn('[vector] ids contain NaN after normalization');
                            if (mask.some((v) => Number.isNaN(v)))
                                console.warn('[vector] mask contain NaN after normalization');
                        }
                        while (ids.length < seqLen)
                            ids.push(0);
                        while (mask.length < seqLen)
                ...
                const feeds = {};
                const ortTensor = (dtype, data, dims) => new ort.Tensor(dtype, data, dims);
                const inputNames = this.session.inputNames ?? ['input_ids', 'attention_mask', 'token_type_ids'];
                // Map common names
                if (inputNames.includes('input_ids'))
                    feeds['input_ids'] = ortTensor('int64', inputIds, [batch, seqLen]);
                if (inputNames.includes('attention_mask'))
                    feeds['attention_mask'] = ortTensor('int64', attnMask, [batch, seqLen]);
                if (inputNames.includes('token_type_ids'))
                    feeds['token_type_ids'] = ortTensor('int64', tokenType, [batch, seqLen]);
                const out = await this.session.run(feeds);
                // Pick first output
                const outName = this.session.outputNames?.[0] ?? Object.keys(out)[0];
                const tensor = out[outName];
                const data = tensor.data; // shape [B, S, H]
                const [B, S, H] = tensor.dims;
                const result = [];
            ...
            this.encodeText = async (text) => (await encodeBatch([text]))[0];
            // Initialize embeddings cache
            this.cache = new EmbeddingsCache(this.dim);
            this.ready = true;
        }
        catch (e) {
            console.warn('[vector] ONNX adapter init failed.', e?.message ?? e);
            this.ready = false;
        }
    }
    async search(query, items, opts) {
        if (!this.ready)
            return [];
        try {
...
export async function getVectorAdapter() {
    ...
    if (cfg.embeddings.mode === 'none')
        return undefined;
    if (!cfg.embeddings.dim || !cfg.embeddings.modelPath)
        return undefined;
    const adapter = new OnnxVectorAdapter(cfg.embeddings.dim);
    await adapter.init();
    if (adapter.ready)
        return adapter;
    return undefined;
