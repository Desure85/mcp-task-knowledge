...
| TR-28 | webhookUrl SSRF-guard в memory async-ops | high | in_progress | TR-03 | register/memory.ts:413,1311 — `z.string().url()` без allowlist; async-ops.ts:277 POSTит job.output на произвольный URL. Нет блока private/loopback/link-local (169.254.169.254), нет scheme-ограничения до https. Эксфильтрация фактов на внутренние сервисы |
