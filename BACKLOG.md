...
| TR-08 | Reliability: деградация и partial failure | medium | in_progress | — | Недокументировано/нетестировано: поведение при downed-коннекторе (hang vs fail-fast), retry/backoff, partial-failure семантика batch-tools, drain endpoint под нагрузкой. Сначала контракт, потом тесты |
