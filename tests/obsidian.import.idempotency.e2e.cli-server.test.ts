...
const TMP = path.join(ROOT, '.tmp-e2e-import-idempotency');
...
describe('obsidian_import_project — idempotency e2e via MCP server', () => {
  ...
  it('first import (replace, confirm) creates data; second import (merge overwrite) creates 0 new items', async () => {
    // 1) replace with confirm
    ...
    // На первом запуске ожидаем хотя бы по одному созданию
    expect((env1.data?.knowledgeImported ?? 0) + (env1.data?.tasksImported ?? 0)).toBeGreaterThan(0);

    // 2) merge overwrite (идемпотентно: новых не создаёт)
    ...
    // На втором запуске создаваемых элементов быть не должно (updates допустимы, но они не считаются как *Imported)
