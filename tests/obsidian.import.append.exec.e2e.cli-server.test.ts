...
const TMP = path.join(ROOT, '.tmp-e2e-import-append');
...
describe('obsidian_import_project — exec append creates duplicates on conflicts', () => {
  ...
  it('baseline replace confirm then append merge leads to >0 new imports', async () => {
    ...
    // second: merge append should create duplicates on same titles
    const res2 = await client!.callTool({
      name: 'obsidian_import_project',
      arguments: { project: PROJECT, knowledge: true, tasks: true, strategy: 'merge', mergeStrategy: 'append' },
    });
    const env2 = parseEnvelope(res2);
    expect(env2.ok).toBe(true);
    // expect new items created due to append strategy
    expect((env2.data?.knowledgeImported ?? 0) + (env2.data?.tasksImported ?? 0)).toBeGreaterThan(0);
  }, 60000);
});
